...
namespace GerenciadorDeTarefas.Controllers
{
    ...
    public class TarefaController : BaseController
    {
        ...
        public IActionResult AdicionarTarefa([FromBody] Tarefa tarefa)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (tarefa.DataPrevistaConclusao == DateTime.MinValue || tarefa.DataPrevistaConclusao < DateTime.Now.Date)
                    {
        ...
        public IActionResult DeletarTarefa(int idTarefa)
        {
            ...
            {
                _logger.LogError("Ocorreu erro ao deletar a tarefa", e);
                return StatusCode(StatusCodes.Status500InternalServerError, new ErroRespostaDto()
                {
                    Status = StatusCodes.Status500InternalServerError,
        ...
        public IActionResult AtualizarTarefa([FromBody] Tarefa model, int idTarefa)
        {
            try
            {
                ...
                {
                    if (!string.IsNullOrEmpty(model.Nome) && !string.IsNullOrWhiteSpace(model.Nome) && model.Nome.Count() < 1)
                    {
                        erros.Add("Favor informar um nome válido");
                    }
                }
            ...
            {
                _logger.LogError("Ocorreu erro ao atualizar a tarefa", e);
                return StatusCode(StatusCodes.Status500InternalServerError, new ErroRespostaDto()
                {
                    Status = StatusCodes.Status500InternalServerError,
        ...
        public IActionResult ListarTarefasUsuario(DateTime? periodoDe, DateTime? periodoAte, StatusTarefaEnum status)
        {
            ...
            {
                _logger.LogError("Ocorreu erro ao listar as tarefas", e);
                return StatusCode(StatusCodes.Status500InternalServerError, new ErroRespostaDto()
                {
                    Status = StatusCodes.Status500InternalServerError,
                    Erro = "Ocorreu erro ao listas as tarefas, tente novamente"
                });
            }
        }
    }
